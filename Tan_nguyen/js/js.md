...
- `Scope` là phạm vi mà một biến hoặc hàm có thể được truy cập trong mã nguồn của chương trình. Trong các ngôn ngữ lập trình, scope được xác định bởi các khối mã, ví dụ như hàm, lớp hoặc module. Các biến được khai báo trong một scope nhất định chỉ có thể được truy cập từ các khối mã trong cùng scope đó hoặc các scope con của nó.

- `Closure` là một khái niệm liên quan đến scope. Nó đề cập đến khả năng của một hàm để truy cập và sử dụng các biến trong scope của nó, cả khi hàm được gọi từ một scope khác. Khi một hàm được định nghĩa trong một scope, nó có thể sử dụng các biến trong scope đó, cũng như các biến ở các scope bao quanh nó. Khi hàm đó được trả về từ scope, các biến trong scope đó sẽ được giữ lại trong bộ nhớ và vẫn có thể được truy cập từ hàm đó khi nó được gọi từ một scope khác.
